 ...
 * put - will only fail due to an external error (e.g. out of memory)
 */
...
int hashmap_put(struct hashmap *map, struct hashmap_key *key, const void *value);

int hashmap_remove(struct hashmap *map, struct hashmap_key *key);
int hashmap_contains_key(const struct hashmap *map, struct hashmap_key *key);
int hashmap_get(const struct hashmap *map, struct hashmap_key *key, void **value);
 ...
 */
int hashmap_for_each(const struct hashmap *map,
